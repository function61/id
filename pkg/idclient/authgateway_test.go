...
func TestAuthUrlContinueToCurrent(t *testing.T) {
	...
	assert.Equal(t, u, "https://function61.com/id?next=https%3A%2F%2Floppi.org%2F_auth%2Fredirect%3Fnext%3D%252Fassets%252Fview%253Fid%253Dfoobar")

	parsed := Must(url.Parse(u))

	nextRaw := parsed.Query().Get("next")
	assert.Equal(t, nextRaw, "https://loppi.org/_auth/redirect?next=%2Fassets%2Fview%3Fid%3Dfoobar")
